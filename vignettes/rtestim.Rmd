...
operator, and $\lambda$ is the penalty which ensures smoothness of the $R_t$ sequence.
...
distribution with values $2.5$ for both the scale and shape parameters,
and the order of the difference operator is set to $3$. A sequence of smoothness 
penalty $\lambda$, if no $\lambda$ is provided, is calculated internally by the
algorithm.
...
$\Rtseq$ with piecewise constant $\Rtseq$ for example, use the command
...
res <- GET(url = "https://api.opencovid.ca/timeseries?",
           query = list('fmt' = 'csv',
                        'geo' = 'can',
                        'stat' = 'cases'))
...
```{r}
mod_cv <- cv_estimate_rt(observed_counts = covid_ca)
```
