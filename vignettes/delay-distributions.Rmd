...
preceding $a$ days of new infections with the discretized generation (or serial) interval distribution $w$ of length $t$. It is the allowable distributions $\{w\}_{a=1}^n$ that are the focus.
...
interval distribution, the we can specify our own manually. For example,
...
n <- nrow(cancovid)
backer_delay <- rev(c(0, delay, rep(0, n - length(delay) - 1)))
delay_mat <- matrix(0, n, n)
delay_mat[1,1] <- 1
for (iter in 2:n) delay_mat[iter, 1:iter] <- backer_delay[1:iter]
...
```{r duotang-processing, eval=FALSE, echo=FALSE}
...
Beta <- bind_rows(
  Beta_IP,
  bonehead_meta |> filter(type == "Alpha", para != "IP") |> mutate(type = "Beta")
)
